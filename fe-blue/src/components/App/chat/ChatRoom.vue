...
<template>
  <div v-if="activeUser" class="flex flex-col flex-1 h-full bg-custom-background relative">
    <!-- Chat Header -->
    <div
      class="flex items-center gap-4 p-6 bg-white border-b border-custom-stroke shrink-0 relative"
    >
      ...
      <div class="size-12 rounded-full overflow-hidden bg-gray-200">
        <img
          :src="activeUser.profile_picture || defaultAvatar"
    ...
    <div ref="messagesContainer" class="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
      <div v-if="loadingMessages && messages.length === 0" class="flex justify-center py-10">
        <div
          class="size-8 border-4 border-custom-blue border-t-transparent rounded-full animate-spin"
        ></div>
      </div>
          ...
          <div
            :class="[
              ...
              String(msg.sender_id) === String(currentUser?.id)
                ...
                : 'bg-white border border-custom-stroke rounded-tl-none text-custom-black'
            ]"
          >
            {{ msg.message }}
            <p
  ...
  <div
    v-else
    class="flex flex-col flex-1 h-full items-center justify-center bg-custom-background text-custom-grey"
  >
