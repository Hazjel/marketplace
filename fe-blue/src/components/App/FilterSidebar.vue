...
<template>
    <div class="flex flex-col gap-6 w-full md:w-[250px] md:min-w-[250px]">
        ...
        <div
            ...
            <!-- Harga -->
            <div class="border-b border-custom-stroke/50 dark:border-white/5 last:border-0 pb-4 last:pb-0">
                <button @click="toggleSection('price')" class="flex justify-between items-center w-full mb-3">
                    ...
                    <i :class="openSections.price ? 'rotate-180' : ''"
                        class="transition-transform fa-solid fa-chevron-down text-xs"></i>
                </button>
                <div v-if="openSections.price" class="flex flex-col gap-3">
                    <div class="flex rounded-lg shadow-sm">
                        ...
                        <input type="number" v-model="priceMin" placeholder="Termurah"
                            class="rounded-none rounded-r-lg bg-white dark:bg-transparent border border-custom-stroke dark:border-white/10 text-custom-black placeholder:text-custom-grey focus:ring-custom-blue focus:border-custom-blue block flex-1 min-w-0 w-full text-sm p-2.5">
                    </div>
                    <div class="flex rounded-lg shadow-sm">
                        <span
                            class="inline-flex items-center px-3 rounded-l-lg border border-r-0 border-custom-stroke dark:border-white/10 bg-gray-50 dark:bg-white/5 text-custom-grey text-xs font-bold">
            ...
            <!-- Kondisi -->
            <div class="border-b border-custom-stroke/50 dark:border-white/5 last:border-0 pb-4 last:pb-0 pt-4">
                <button @click="toggleSection('condition')" class="flex justify-between items-center w-full mb-3">
                    ...
                    <i :class="openSections.condition ? 'rotate-180' : ''"
                        class="transition-transform fa-solid fa-chevron-down text-xs"></i>
                </button>
                <div v-if="openSections.condition" class="flex flex-col gap-2">
                    <label v-for="cond in conditions" :key="cond.value" class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" :value="cond.value" v-model="selectedConditions"
                            class="w-4 h-4 text-custom-blue rounded focus:ring-custom-blue">
            ...
            <div class="border-b border-custom-stroke/50 dark:border-white/5 last:border-0 pb-4 last:pb-0 pt-4">
                <button @click="toggleSection('rating')" class="flex justify-between items-center w-full mb-3">
                    ...
                    <i :class="openSections.rating ? 'rotate-180' : ''"
                        class="transition-transform fa-solid fa-chevron-down text-xs"></i>
                </button>
                <div v-if="openSections.rating" class="flex flex-col gap-2">
                    <label v-for="level in ratingLevels" :key="level" class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" :true-value="level" :false-value="null" v-model="minRating"
                            class="w-4 h-4 text-custom-blue rounded focus:ring-custom-blue">
                        <div class="flex items-center gap-2">
                            <div class="flex gap-0.5">
                                <i v-for="n in level" :key="n" class="fa-solid fa-star text-xs"
            ...
            <div class="pt-4">
                <button @click="toggleSection('added')" class="flex justify-between items-center w-full mb-3">
                    ...
                    <i :class="openSections.added ? 'rotate-180' : ''"
                        class="transition-transform fa-solid fa-chevron-down text-xs"></i>
                </button>
                <div v-if="openSections.added" class="flex flex-wrap gap-2">
                    <button v-for="opt in timeOptions" :key="opt.value"
                        ...
                        :class="createdSince === opt.value ? 'bg-custom-blue/10 text-custom-blue border-custom-blue' : 'bg-white dark:bg-transparent text-custom-grey border-custom-stroke dark:border-white/10 hover:border-custom-blue'"
