...
<template>
    <div class="w-full relative rounded-[32px] overflow-hidden bg-white/70 backdrop-blur-xl border border-white/50 shadow-xl transition-all duration-300">
        ...
        <div class="relative z-10 p-6 md:p-8 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
            <!-- Left: Store Info -->
            <div class="flex items-center gap-6 flex-1 min-w-0">
                <!-- Logo with Ring -->
                <div class="relative">
                    <div class="w-[88px] h-[88px] rounded-full p-[3px] bg-gradient-to-br from-custom-blue to-purple-400 shadow-lg">
                    ...
                    <div v-if="isVerified" class="absolute -bottom-1 -right-1 bg-white rounded-full p-1 shadow-sm">
                        <img src="@/assets/images/icons/verify-star.svg" class="w-6 h-6" alt="Verified">
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-2 flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                         <h1 class="font-bold text-2xl md:text-3xl text-custom-black truncate leading-tight tracking-tight">{{ storeName }}</h1>
                    </div>
                    
                    <div class="flex items-center gap-4 text-custom-grey text-sm font-medium flex-wrap">
                        <div class="flex items-center gap-1.5">
                            <img src="@/assets/images/icons/location-grey.svg" class="w-4 h-4 opacity-70" alt="Loc">
                            <span>{{ storeLocation }}</span>
                        </div>
                        <div class="w-1 h-1 rounded-full bg-gray-300"></div>
                        <div class="flex items-center gap-1.5" :class="{'text-green-500': true}">
                            <div class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                            <span>Online 5 mins ago</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-6 mt-1">
                        <div class="flex items-center gap-1.5" title="Rating">
                            <img src="@/assets/images/icons/Star-pointy.svg" class="w-5 h-5" alt="Star">
                            <span class="font-bold text-custom-black">{{ rating.toFixed(1) }}</span>
                            <span class="text-xs text-custom-grey">/ 5.0</span>
                        </div>
                        <div class="w-[1px] h-4 bg-gray-200"></div>
                        <div class="flex flex-col leading-none">
                            <span class="font-bold text-custom-black">{{ followersCount }}</span>
                            <span class="text-[10px] text-custom-grey uppercase tracking-wide">Followers</span>
                        </div>
                         <div class="w-[1px] h-4 bg-gray-200"></div>
                        <div class="flex flex-col leading-none">
                            <span class="font-bold text-custom-black">98%</span>
             ...
             <div class="flex flex-col gap-4 w-full md:w-auto items-end">
                <div class="flex items-center gap-3 w-full md:w-auto">
                    ...
                    <button @click="handleFollowClick" 
                        class="flex-1 md:flex-none h-11 px-6 rounded-full font-bold transition-all duration-300 flex items-center justify-center gap-2 border"
                        :class="isFollowing 
                    ...
                    <!-- Chat Button -->
                     <RouterLink v-if="authStore.user"
                        ...
                        class="h-11 w-11 flex items-center justify-center rounded-full bg-white border border-gray-200 text-custom-blue hover:border-custom-blue hover:bg-blue-50 transition-all shadow-sm">
                    ...
                    <RouterLink v-else :to="{ name: 'auth.login' }"
                     ...
                     <!-- Share Button -->
                    <button class="h-11 w-11 flex items-center justify-center rounded-full bg-white border border-gray-200 text-gray-500 hover:text-black hover:border-gray-300 transition-colors shadow-sm">
                ...
                <div class="relative w-full md:w-[320px]">
                    ...
                    <input type="text" placeholder="Search in this store..." 
                           class="w-full h-11 pl-11 pr-4 rounded-full bg-white/50 border border-white/60 focus:bg-white focus:border-custom-blue focus:ring-2 focus:ring-blue-100 outline-none text-sm transition-all shadow-sm placeholder-gray-400">
                </div>
            </div>
        </div>
    </div>
