...
<template>
  <li
    v-if="!item.children && can(item.permission) && hasRole(item.role)"
    class="group"
  ...
  >
    ...
    >
      ...
      <img
        src="@/assets/images/icons/arrow-circle-up-black.svg"
    ...
    <div
      v-show="isOpen"
      :id="`accordion-${(item.path || item.label || 'default').replace(/[\/\s]/g, '-')}`"
      class="flex"
    >
      <div class="flex w-[56px] shrink-0 justify-end items-start relative">
        <div
          ...
          :key="n"
          class="w-[2px] h-full bg-gray-200 absolute top-0 left-1/2 -translate-x-1/2 rounded-[1px] last:h-[55%]"
        ></div>
      </div>
      <ul class="flex flex-col gap-1 w-full">
        ...
        >
          <div
            ...
            class="absolute left-0 top-0 w-[28px] h-[32px] border-l-2 border-b-2 border-gray-200 rounded-bl-2xl bg-transparent z-[1] -translate-x-[104%]"
          ></div>
          <RouterLink
            v-if="can(child.permission) && hasRole(child.role)"
            :to="child.path"
          ...
          >
            ...
            <p
              class="font-medium text-custom-grey group-[&.active]:text-custom-blue transition-300 w-full"
            >
              {{ child.label }}
            </p>
            <div
