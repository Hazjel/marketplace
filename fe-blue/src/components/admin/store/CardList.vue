    ...
    <div class="card flex flex-col rounded-[20px] border border-custom-stroke py-[18px] px-5 gap-5 bg-white">
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div class="flex flex-col md:flex-row items-center justify-end gap-[14px] w-full md:w-auto">
                <button class="flex items-center justify-center h-14 w-full md:w-[126px] shrink-0 rounded-2xl p-4 gap-2 bg-custom-red/10" v-if="can('store-delete')" @click="emit('delete', item.id)" >
                ...
                <RouterLink :to="{ name: 'admin.store.detail', params: { id: item.id }}" class="flex items-center justify-center h-14 w-full md:w-[126px] shrink-0 rounded-2xl p-4 gap-2 bg-custom-blue">
