    ...
    setTimeout(() => {
        new Swiper('.gallery-swiper', {
            ...
            pagination: {
                ...
                bulletClass: 'swiper-pagination-bullet !w-1.5 !h-1.5 !bg-gray-300 !opacity-100 !rounded-full !transition-all !duration-300'
            },
        });
    }, 100);
};
...
<template>
    <div class="flex flex-col w-full gap-4">
        ...
        <div class="hidden md:flex flex-row-reverse w-full gap-4 h-[450px]">
            <!-- Main Image Area -->
            <div class="relative flex-1 bg-custom-background rounded-2xl overflow-hidden cursor-zoom-in group"
                @mouseenter="toggleZoom(true)" @mouseleave="toggleZoom(false)" @mousemove="handleMouseMove"
                @click="showLightbox = true">
                <img :src="activeImage?.image"
                    class="size-full object-contain transition-transform duration-100 ease-out" :style="zoomStyle"
                    alt="product detail">
            ...
            <div class="flex flex-col gap-3 w-[100px] h-full overflow-y-auto no-scrollbar py-1">
                <button
                    ...
                    v-for="image in images" :key="image.id" :class="{ 'active': image.image === activeImage?.image }"
        ...
        <div v-if="showLightbox"
            ...
            <div class="w-full h-full max-w-5xl max-h-[90vh] p-4 flex items-center justify-center">
                <img :src="activeImage?.image" class="max-w-full max-h-full object-contain" alt="fullscreen">
            </div>

            <!-- Lightbox Thumbnails -->
            <div
                class="absolute bottom-10 left-1/2 -translate-x-1/2 flex gap-4 overflow-x-auto max-w-[90vw] pb-2 hide-scrollbar">
                <button v-for="image in images" :key="image.id"
                    class="size-16 rounded-lg overflow-hidden border-2 transition-all opacity-50 hover:opacity-100"
                    :class="image.image === activeImage?.image ? 'border-white opacity-100' : 'border-transparent'"
