...
<template>
    <div class="flex flex-col gap-6 max-w-2xl mx-auto">
        <div class="flex items-center gap-4">
            <RouterLink :to="{ name: 'user.settings.address' }" 
        ...
        <form @submit.prevent="submit" class="flex flex-col gap-6 bg-white p-6 md:p-8 rounded-3xl border border-custom-stroke shadow-sm">
            
            <!-- Label -->
            <div class="flex flex-col gap-2">
                <label class="font-bold text-custom-black">Address Label</label>
                <input type="text" v-model="form.label" required placeholder="e.g. Home, Office"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="text" v-model="form.recipient_name" required placeholder="Full Name"
                        class="w-full h-12 px-4 rounded-xl border border-custom-stroke focus:border-custom-black focus:ring-0 outline-none transition-all" />
                </div>
                <!-- Phone -->
                <div class="flex flex-col gap-2">
            ...
            <div class="flex flex-col gap-2 relative">
                ...
                <div class="relative">
                    <input type="text" v-model="citySearch" @input="handleCityInput(citySearch)" required 
                    ...
                    <div v-if="loadingCities" class="absolute right-3 top-3.5">
                        <div class="size-5 border-2 border-custom-grey border-t-transparent rounded-full animate-spin"></div>
                    </div>
                </div>
                
                <!-- Dropdown -->
                <div v-if="showCityOptions" class="absolute top-full left-0 w-full mt-2 bg-white border border-custom-stroke rounded-xl shadow-xl z-50 max-h-60 overflow-y-auto">
                    <div v-for="city in cityOptions" :key="city.id" @click="selectCity(city)"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="flex flex-col gap-2 md:col-span-2">
                    ...
                    <textarea v-model="form.address" required rows="3" placeholder="Street name, house number, etc."
                        class="w-full p-4 rounded-xl border border-custom-stroke focus:border-custom-black focus:ring-0 outline-none transition-all resize-none"></textarea>
                </div>
                <!-- Postal Code -->
                <div class="flex flex-col gap-2">
                    <label class="font-bold text-custom-black">Postal Code</label>
                    <input type="text" v-model="form.postal_code" required placeholder="12345"
                        class="w-full h-12 px-4 rounded-xl border border-custom-stroke focus:border-custom-black focus:ring-0 outline-none transition-all" />
                </div>
            </div>
            ...
            <label class="flex items-center gap-3 cursor-pointer group">
                ...
                <div class="w-[60px] h-8 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-[28px] after:w-[28px] after:transition-all peer-checked:bg-custom-blue relative"></div>
                <span class="font-bold text-custom-black group-hover:text-custom-blue transition-colors">Set as Primary Address</span>
            </label>
            ...
            <button type="submit" :disabled="submitting" 
                ...
                <div v-if="submitting" class="size-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
