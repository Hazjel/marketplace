    ...
    <main class="w-full max-w-[1280px] px-4 md:px-[52px] mt-8 mb-[100px] mx-auto">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-5 flex flex-col gap-6">
                ...
                <div class="sticky-tabs flex items-center gap-6 overflow-x-auto hide-scrollbar bg-white -mx-4 px-4 md:mx-0 md:px-0">
                ...
                <!-- Store Info -->
                 <div class="flex items-center gap-3">
                    ...
                    <RouterLink v-if="product?.store?.username" :to="{ name: 'app.store-detail', params: { username: product?.store?.username } }" class="ml-auto px-4 py-1.5 border border-custom-blue text-custom-blue rounded-lg text-sm font-bold hover:bg-blue-50">
                        Follow
                    </RouterLink>
                 </div>
            ...
            <div class="lg:col-span-3">
                 <div class="sticky top-[120px] bg-white rounded-2xl p-5 shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100 flex flex-col gap-5">
                     ...
                     <div class="flex items-center gap-3 my-2">
                        <div class="size-12 rounded bg-gray-100 overflow-hidden shrink-0">
                             <img :src="product?.product_images?.find(img => img.is_thumbnail)?.image" class="size-full object-cover" />
                        </div>
                     ...
                     <div class="flex items-center gap-2 border border-custom-stroke rounded-lg p-1 w-fit">
                        <button type="button" @click="decrease" :disabled="quantity <= 1" class="size-7 flex items-center justify-center text-custom-blue hover:bg-gray-50 rounded disabled:text-gray-300">
                             <i class="fa-solid fa-minus text-xs"></i>
                        </button>
                        <input type="number" v-model="quantity" readonly class="w-10 text-center font-bold text-sm outline-none" />
                         <button type="button" @click="increase" :disabled="quantity >= (product?.stock || 0)" class="size-7 flex items-center justify-center text-custom-blue hover:bg-gray-50 rounded disabled:text-gray-300">
                             <i class="fa-solid fa-plus text-xs"></i>
                        </button>
                     </div>
                     <p class="text-xs text-custom-orange" v-if="product?.stock < 5">Sisa {{ product?.stock }} buah lagi!</p>
                     <p class="text-xs text-custom-grey" v-else>Stok Total: {{ product?.stock }}</p>

                     <div class="flex items-center justify-between mt-2">
                        <span class="text-custom-grey">Subtotal</span>
                        <span class="font-bold text-lg">Rp {{ formatRupiah((product?.price || 0) * quantity) }}</span>
                     </div>
                    
                     <div class="flex flex-col gap-2 mt-2">
                        <button @click.prevent="addToCart" :disabled="!product?.stock || product?.stock <= 0" class="w-full py-3 bg-custom-blue text-white rounded-lg font-bold hover:bg-blue-600 disabled:bg-gray-300 transition-colors flex items-center justify-center gap-2">
                             <i class="fa-solid fa-plus"></i> Keranjang
                        </button>
                        <button class="w-full py-3 border border-custom-blue text-custom-blue rounded-lg font-bold hover:bg-blue-50 transition-colors">
                            Beli Langsung
                        </button>
                     </div>

                     <div class="flex items-center justify-center gap-4 mt-2">
                        <button @click="handleToggleWishlist" class="flex items-center gap-2 text-sm font-bold text-custom-grey hover:text-custom-red transition-colors">
                            <i class="fa-solid fa-heart" :class="isInWishlist ? 'text-custom-red' : ''"></i> Wishlist
                        </button>
                        <button class="flex items-center gap-2 text-sm font-bold text-custom-grey hover:text-custom-black transition-colors">
        ...
        <div class="fixed bottom-0 left-0 w-full z-50 bg-white border-t border-custom-stroke p-4 md:hidden shadow-[0_-4px_20px_-4px_rgba(0,0,0,0.1)] flex gap-3 items-center">
