...
<template>
    <div class="flex flex-col gap-5">
        ...
        <section class="flex flex-col w-[580px] shrink-0 rounded-[20px] py-5 gap-5 bg-white">
            ...
            <RouterLink :to="{ name: 'admin.category.create', query: { parent_id: productCategory?.id }}" class="flex h-14 items-center justify-center rounded-2xl py-4 px-6 bg-custom-blue gap-[6px] mx-5">
                ...
                <template v-for="childrens in productCategory?.childrens">
                    ...
                    <div class="flex items-center gap-[10px] w-full">
                        <div class="flex size-[72px] p-[15px] shrink-0 bg-white overflow-hidden items-center justify-center">
                            <img :src="childrens.image ?? PlaceHolder" class="flex size-full object-contain" alt="icon">
                        </div>
                        <div class="flex flex-col gap-1">
                    ...
                    <div class="flex items-center gap-[14px]">
                        <button class="flex items-center justify-center size-14 shrink-0 rounded-2xl p-4 gap-2 bg-custom-red/10" @click="handleDelete(childrens.id)">
                            <img src="@/assets/images/icons/trash-red.svg" class="flex size-6 shrink-0" alt="icon">
                        </button>
                        <RouterLink :to="{ name: 'admin.category.edit', params: { id: childrens.id }, query: { parent_id: productCategory.id }}" class="flex items-center justify-center size-14 shrink-0 rounded-2xl p-4 gap-2 bg-custom-black">
