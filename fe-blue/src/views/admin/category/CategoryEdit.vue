  ...
  <div class="flex flex-col gap-6">
    ...
    <form class="flex flex-col w-full rounded-2xl border border-gray-100 dark:border-white/10 bg-white dark:bg-surface-card shadow-sm p-6 gap-6" @submit.prevent="handleSubmit">
      ...
      <div v-if="productCategory.parent_id" class="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        ...
        <div class="group/errorState flex flex-col gap-2 w-full md:w-1/2" :class="{ invalid: error?.parent_id }">
          <label class="group relative">
            ...
            <p class="input-placeholder">Kategori Induk</p>
            <input
          ...
          <span v-if="error?.parent_id" class="input-error">{{ error?.parent_id?.join(', ') }}</span>
        </div>
      </div>
      ...
      <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        ...
        <div class="group/errorState flex flex-col gap-2 w-full md:w-1/2" :class="{ invalid: error?.name }">
          <label class="group relative">
            ...
            <p class="input-placeholder">Masukkan Nama Kategori</p>
            <input v-model="productCategory.name" type="text" class="custom-input" placeholder="" />
          </label>
      ...
      <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        ...
        <div class="group/errorState flex flex-col gap-2 w-full md:w-1/2" :class="{ invalid: error?.tagline }">
          <label class="group relative">
            <div class="input-icon">
              <svg xmlns="http://www.w3.org/2000/svg" class="size-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
            ...
            <p class="input-placeholder">Masukkan Tagline</p>
            <input v-model="productCategory.tagline" type="text" class="custom-input" placeholder="" />
          </label>
          <span v-if="error?.tagline" class="input-error">{{ error?.tagline?.join(', ') }}</span>
        </div>
      </div>
