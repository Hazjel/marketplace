...
const validateStep = (step) => {
    ...
    if (step === 3) {
        const hasThumbnail = product.value.product_images.some(img => img.is_thumbnail && (img.image || img.url !== PlaceHolder));
        // Also check if at least one real image exists
        const hasImage = product.value.product_images.some(img => img.image || img.url !== PlaceHolder);
...
<template>
    <div class="flex flex-col w-full rounded-3xl p-6 gap-6 bg-white min-h-[600px]">
        <StepWizard :steps="steps" :current-step="currentStep"
            @change-step="(s) => { if (validateStep(currentStep)) currentStep = s }" />

        <form @submit.prevent="handleSubmit" class="flex flex-col gap-6 flex-1">
                        ...
                        <label
                            ...
                            <div class="flex flex-col gap-[6px] pl-4 w-full">
                                ...
                                <textarea class="appearance-none outline-none w-full font-semibold leading-[160%]"
                                    rows="3" placeholder="" v-model="product.description"></textarea>
                            </div>
                        </label>
                        <span class="input-error" v-if="error?.description">{{ error?.description?.join(', ') }}</span>
                    </div>
                </div>
                    ...
                    <div class="flex flex-col md:flex-row gap-4 w-full">
                        <div class="group/errorState flex flex-col gap-2 w-full">
                            <label
                                ...
                                <div class="input-icon">
                                    <img src="@/assets/images/icons/bag-grey.svg" class="flex size-6 shrink-0"
                                        alt="icon">
                                </div>
                                <select required
                                    ...
                                    <option v-for="category in productCategories" :value="category.id">{{ category.name
                                    }}</option>
                                </select>
                                <p class="placeholder font-bold absolute -translate-y-1/2 left-[81px] top-[25px] transition-300"
                                    :class="product.parent_product_category_id ? 'text-sm text-custom-grey font-semibold' : ''">
                                    Select Category</p>
                                <img src="@/assets/images/icons/arrow-down-black.svg"
                                    class="absolute transform -translate-y-1/2 top-1/2 right-6 size-6" alt="icon">
                            </label>
                        </div>
                        <div class="group/errorState flex flex-col gap-2 w-full" v-if="subCategories.length > 0">
                            <label
                                class="group relative rounded-[18px] border-[1.5px] border-custom-stroke focus-within:border-custom-black transition-300 overflow-hidden w-full">
                                <div class="input-icon">
                                    <img src="@/assets/images/icons/shopping-bag-black.svg" class="flex size-6 shrink-0"
                                        alt="icon">
                                </div>
                                <select required
                                    class="appearance-none w-full h-[72px] font-semibold text-lg outline-none pl-20 pr-6 pb-[14.5px] pt-[32px] bg-transparent z-10 relative"
                                    ...
                                    <option hidden></option>
                                    <option v-for="category in subCategories" :value="category.id">{{ category.name }}
                                    </option>
                                </select>
                                <p class="placeholder font-bold absolute -translate-y-1/2 left-[81px] top-[25px] transition-300"
                                    :class="product.product_category_id ? 'text-sm text-custom-grey font-semibold' : ''">
                                    Select Sub Category</p>
                                <img src="@/assets/images/icons/arrow-down-black.svg"
                                    class="absolute transform -translate-y-1/2 top-1/2 right-6 size-6" alt="icon">
                            </label>
                        </div>
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div class="grid grid-cols-2 gap-6 h-[76px] w-full">
                        <label
                            ...
                            <input type="radio" name="condition" class="absolute opacity-0" value="new"
                        ...
                        <label
                            ...
                            <input type="radio" name="condition" class="absolute opacity-0" value="second"
                                v-model="product.condition">
                        </label>
                    </div>
                </div>
            ...
            <div v-show="currentStep === 2" class="flex flex-col gap-6 animate-fade-in-up">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div class="group/errorState flex flex-col gap-2 w-full" :class="{ 'invalid': error?.weight }">
                        <label class="group relative">
                            ...
                            <input type="number" class="custom-input" placeholder="" v-model="product.weight">
                        </label>
                    </div>
                </div>
                ...
                <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    <div class="w-full md:w-1/3 pt-2">
                        ...
                        <p class="text-xs text-custom-grey mt-1">Does this product have options like size, color?</p>
                    </div>
                    ...
                    <div class="w-full flex flex-col gap-4">
                        ...
                        <div v-if="product.has_variants"
                            class="p-4 border rounded-xl bg-blue-50/50 flex flex-col gap-4">
                            <span class="text-xs font-bold text-gray-500 uppercase">1. Define Options</span>
                            <div class="flex gap-2">
                                <input type="text" v-model="newOptionName" placeholder="Add Option (e.g. Color)"
                                    class="custom-input h-10 text-sm bg-white">
                                <button type="button" @click="addOptionGroup"
                                    class="h-10 px-6 text-sm font-bold bg-custom-black text-white rounded-lg hover:bg-gray-800 transition-colors">Add</button>
                            </div>
                            <div class="flex flex-wrap gap-2">
                                <span v-for="(opt, idx) in optionGroups" :key="idx"
                                    ...
                                    {{ opt }}
                                    <button type="button" @click="optionGroups.splice(idx, 1)"
                                        class="hover:text-red-500 text-lg leading-none">&times;</button>
                                </span>
                            </div>
                        </div>
                        ...
                        <div v-if="product.has_variants" class="flex flex-col gap-3">
                            ...
                            <div v-for="(variant, index) in product.variants" :key="index"
                                ...
                                <div class="flex justify-between items-center border-b pb-2 mb-2">
                                    ...
                                    <button type="button" @click="product.variants.splice(index, 1)"
                                ...
                                <div v-if="optionGroups.length > 0" class="grid grid-cols-2 gap-3">
                                    <div v-for="opt in optionGroups" :key="opt" class="flex flex-col gap-1">
                                        ...
                                        <input type="text" v-model="variant.variant_attributes[opt]"
                                ...
                                <div class="flex flex-col gap-1">
                                    ...
                                    <input type="text" v-model="variant.name"
                                        class="custom-input h-10 text-sm bg-gray-50" placeholder="Name">
                                </div>

                                <div class="flex gap-3">
                                    <div class="flex flex-col gap-1 w-1/2">
                                        <label class="text-xs font-semibold text-gray-400">Price (Rp)</label>
                                        <input type="number" v-model="variant.price" class="custom-input h-10 text-sm"
                                            placeholder="Price">
                                    </div>
                                    <div class="flex flex-col gap-1 w-1/2">
                                        <label class="text-xs font-semibold text-gray-400">Stock</label>
                                        <input type="number" v-model="variant.stock" class="custom-input h-10 text-sm"
                                            placeholder="Stock">
                                    </div>
                                </div>
                            </div>
                ...
                <div class="flex flex-col md:flex-row justify-between gap-4" v-if="!product.has_variants">
                    ...
                    <div class="flex gap-6 w-full">
                        <div class="group/errorState flex flex-col gap-2 w-1/2" :class="{ 'invalid': error?.price }">
                            <label class="group relative">
                                <div class="input-icon">
                                    <img src="@/assets/images/icons/money-grey.svg" class="flex size-6 shrink-0"
                                        alt="icon">
                                </div>
                        ...
                        <div class="group/errorState flex flex-col gap-2 w-1/2" :class="{ 'invalid': error?.stock }">
                            <label class="group relative">
                                <div class="input-icon">
                                    <img src="@/assets/images/icons/box-grey.svg" class="flex size-6 shrink-0"
                                        alt="icon">
                                </div>
            ...
            <div v-show="currentStep === 3" class="flex flex-col gap-6 animate-fade-in-up">
                <div class="flex flex-col justify-between gap-4">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full">
                        <div class="thumbnail-input-container group relative flex w-full aspect-square rounded-2xl overflow-hidden items-center justify-center bg-gray-50 border-2 border-dashed border-gray-200 hover:border-custom-blue transition-colors"
                            ...
                            <img :src="image.url"
                                :class="image.url === PlaceHolder ? 'w-8 h-8 opacity-40' : 'size-full object-cover'"
                                class="thumbnail pointer-events-none" alt="icon" />

                            <div class="absolute bottom-0 left-0 w-full p-2 bg-black/50 text-white text-[10px] text-center backdrop-blur-sm"
                                v-if="index === 0">
                                Thumbnail
                            </div>
                        </div>
            ...
            <div class="flex items-center justify-between pt-6 mt-4 border-t border-gray-100">
                <button type="button" v-if="currentStep > 1" @click="prevStep"
                    class="h-12 px-6 rounded-full font-bold text-gray-500 hover:bg-gray-100 transition-colors">
                    Back
                </button>
