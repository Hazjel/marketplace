    ...
    <div class="flex flex-col flex-1 rounded-[20px] p-5 gap-6 bg-white">
        ...
        <section id="List-Transactions" class="flex flex-col flex-1 gap-6 w-full">
            <div class="list flex flex-col gap-5">
                <CardList v-for="storeBalance in storeBalances" :key="storeBalance.id" :item="storeBalance" v-if="!loading && storeBalances" />
            </div>
            <Pagination :meta="meta" :server-options="serverOptions"/>
        </section>
        <div id="Empty-State" class="flex flex-col flex-1 items-center justify-center gap-4" v-if="storeBalances?.length === 0">
