...
const handleAddressInput = debounce(async (search) => {
    ...
    try {
        ...
        const response = await axiosRaw.get(`/tariff/api/v1/destination/search?keyword=${encodeURIComponent(search)}`, {
             headers: {
    ...
    <div class="w-full min-h-screen lg:grid lg:grid-cols-2">
        ...
        <div class="flex flex-col justify-center px-6 py-12 lg:px-16 xl:px-24 bg-white overflow-y-auto">
            <div class="w-full max-w-xl mx-auto">
                <form @submit.prevent="handleSubmit" autocomplete="off" class="flex flex-col gap-8">
                    
                    <div class="flex flex-col gap-2">
                        <h1 class="font-bold text-3xl lg:text-4xl text-custom-black">Start Selling Today! 🚀</h1>
                    ...
                    <div class="flex flex-col gap-6">
                        ...
                        <div class="flex flex-col gap-3">
                            ...
                            <div class="group/errorState flex flex-col gap-2 relative">
                                ...
                                <!-- Dropdown Results -->
                                <ul v-if="showAddressOptions" class="absolute top-full mt-2 w-full bg-white border border-custom-stroke rounded-xl shadow-xl max-h-[300px] overflow-y-auto z-50">
                                    <li v-if="loadingAddress" class="p-4 text-center text-custom-grey font-medium">Loading...</li>
                                    <li v-else v-for="option in addressOptions" :key="option.id"
                                    ...
                                    <li v-if="!loadingAddress && addressOptions.length === 0 && addressSearch" class="p-4 text-center text-custom-grey font-medium">
                                        No results found
                                    </li>
                                </ul>
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="flex flex-col gap-3">
                                ...
                                <div class="group/errorState flex flex-col gap-2" :class="{ 'invalid': errors?.city }">
                                    <label class="group relative">
                                        <div class="input-icon">
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                <div class="group/errorState flex flex-col gap-2" :class="{ 'invalid': errors?.postal_code }">
                                    <label class="group relative">
                                        <div class="input-icon">
                        ...
                        <div class="flex flex-col gap-3">
                            ...
                            <div class="group/errorState flex flex-col gap-2" :class="{ 'invalid': errors?.address }">
                                <label class="group relative">
                    ...
                    <div class="flex flex-col gap-3 mt-4">
                        ...
                        <RouterLink :to="{ name: 'app.home' }" class="text-center font-medium text-custom-grey hover:text-custom-black">
                            Cancel
                        </RouterLink>
                    </div>
                </form>
        ...
        <div class="hidden lg:block relative sticky top-0 h-screen bg-gray-50">
             <div v-if="form.city" class="w-full h-full relative">
                ...
                <!-- Floating Info Card -->
                <div class="absolute top-8 left-8 z-10 bg-white/95 backdrop-blur-md p-5 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white/20 flex flex-col gap-4 min-w-[260px] max-w-[320px]">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-custom-background flex items-center justify-center shrink-0">
                            <img src="@/assets/images/icons/location-grey.svg" class="w-5 h-5" alt="city">
                        </div>
                        <div class="flex flex-col">
                             <p class="text-[11px] font-bold text-custom-grey uppercase tracking-wider mb-0.5">City Location</p>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex flex-col">
                             <p class="text-[11px] font-bold text-custom-grey uppercase tracking-wider mb-0.5">Postal Code</p>
                            <p class="font-bold text-custom-black text-lg leading-tight">{{ form.postal_code }}</p>
                        </div>
                    </div>
                </div>
             </div>
            <div v-else class="flex flex-col items-center justify-center h-full text-custom-grey gap-6 p-10 text-center">
                <div class="w-20 h-20 rounded-full bg-white flex items-center justify-center shadow-sm">
                     <img src="@/assets/images/icons/location-grey.svg" class="w-10 h-10 opacity-50" alt="location">
                </div>
                <div>
                     <h3 class="font-bold text-xl text-custom-black mb-2">Locate Your Store</h3>
                    <p class="font-medium text-gray-400 max-w-xs mx-auto">Enter your city or district in the search bar to see your store location on the map.</p>
                </div>
            </div>
        </div>
    </div>
